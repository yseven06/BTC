...
# SYMBOL ROTATION PERIOD — prime, and deliberately larger than UNIVERSE_CAP.
#
# WHY A PRIME, AND WHY NOT `bucket % count`. `bucket % count` is only fair when
# the executed buckets advance one at a time. If the job executes on a fixed
# STRIDE s (every other cadence, every fourth, ...), the offsets it can produce
# are the coset generated by gcd(s, count): with count = 56 and s = 4 only 14 of
# the 56 offsets ever occur, and 42 symbols are never reached — measured, not
# hypothesised. Any modulus sharing a factor with the stride freezes the phase.
#
# 251 is prime and 251 > UNIVERSE_CAP, which removes both failure modes at once:
#   * gcd(s, 251) = 1 for EVERY stride s that is not a multiple of 251, so
#     j -> (b0 + j*s) mod 251 is a bijection of Z_251. Over any 251 consecutive
#     EXECUTED runs, `bucket % 251` therefore takes every value in 0..250.
#   * count <= UNIVERSE_CAP = 250 < 251, so x -> x % count restricted to
#     0..250 is onto 0..count-1.
# Composing them: every offset — in particular the one that puts a given symbol
# FIRST — occurs at least once per 251 executed runs, for any universe size, any
# number of symbols reached per run, and any stride that is not a multiple of
# 251. The excluded case is stated in `rotation_offset`.
SYMBOL_ROTATION_PERIOD = 251
...
def rotation_offset(count: int, *, now: Optional[datetime] = None,
                    ...
                    period: int = SYMBOL_ROTATION_PERIOD) -> int:
    ...
    WHY THE PRIME PROJECTION, AND NOT `bucket % count`. `bucket % count` is fair
    only when executed buckets advance one at a time. Under a fixed execution
    STRIDE s the reachable offsets collapse to the coset generated by
    gcd(s, count): measured on the previous commit, count = 56 with s = 4 reached
    14 of 56 symbols and never reached the other 42, forever. Projecting through
    a prime larger than any universe removes the shared factor.

    THE BOUND. Let the executed buckets be b_j = b0 + j*s. Because `period` is
    prime and does not divide s, gcd(s, period) = 1, so j -> b_j mod period is a
    bijection of Z_period: over any `period` consecutive EXECUTED runs it takes
    every value in 0..period-1. Because count <= UNIVERSE_CAP < period, reducing
    those values mod count is onto 0..count-1. Hence EVERY offset — including the
    one that puts any given symbol FIRST — occurs at least once per `period`
    executed runs. This holds for every universe size, every number of symbols
    reached per run, and every stride, with one exception.

    THE EXCLUDED CASE, stated rather than hidden: a stride that is an exact
    multiple of `period` (251 cadence buckets, i.e. one execution per ~5.2 days,
    sustained forever) leaves b_j mod period constant and freezes the offset.
    Arbitrary ADVERSARIAL skip sequences are likewise outside any stateless
    scheme — an adversary who picks the executed buckets can hold the residue
    fixed. Neither is a property a stateless clock-derived rotation can defeat;
    defeating them requires durable progression state, which this checkpoint is
