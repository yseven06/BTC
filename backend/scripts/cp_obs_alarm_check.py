...
logic; adds NO OS task / cron / worker / push channel / table / dependency.
Deterministic given (DB snapshot, now()): no randomness; the verdict is stable across
back-to-back runs — only the live staleness minutes advance with wall-clock.
...
async def main() -> int:
    try:
        ...
        print(f"verdict=ERROR reason={type(e).__name__}:{str(e)[:80]!r}")
        return EXIT[ERROR]
    ...
    if not present:
        print("verdict=ERROR reason='no telemetry timestamps'")
        return EXIT[ERROR]
    ...
    print(
        ...
        f"dup_tradepath={dup_tp} dup_perf={dup_perf}"
    )
    # short human context (does not change the parse contract above)
    print(
        f"# CP-OBS-ALARM-B liveness: system is {v} "
        f"(freshest telemetry {round(whole,1)}m old; OK<= {WARN_MIN}m, WARN> {WARN_MIN}m, CRIT> {CRIT_MIN}m). "
        f"latest: status_history={str(f['status_max'])[:19]} generated={str(f['gen_max'])[:19]}",
        file=sys.stdout,
    )
    return EXIT[v]
