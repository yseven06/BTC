...
**Tarih:** 2026-07-03 · **Rev:** v1.1 (uygulama sırası revizyonu — kullanıcı onayı) · **Statü:** PLAN (uygulama başlamadı — her paket ayrı onayla başlar)
...
### Uygulama sırası (v1.1 revizyonu — bağlayıcı)
**P1.2 → P1.1 → P1.4 → P1.3 → P1.5 → P1.6.** Gerekçe: (1) Bible display tipografisi **650 ağırlık** ister; mevcut Google `@import` statik kesim yükler (650 yok) — hero önce yapılsa 700'le kurulup next/font variable gelince yeniden ayarlanırdı; temel-önce sırası hero'nun ikinci revizyonunu ortadan kaldırır. (2) VL ışık token'ları (`--light-key`/`--light-fill`) doğru sahibinde (P1.2 token commit'i) tanımlanır. (3) Her ekran final palette **bir kez** görsel-doğrulanır. (4) P1.2→P1.1→P1.4 landing'i tek solukta bitirir; sonra app ekranları.
**Sonuç:** Sprint'in ilk kapısı **K1 (Bible v1.2)** + K3'tür — P1, standart revizyonu onayıyla açılır. **Paket bitmeden sonraki pakete geçilmez** (kullanıcı kuralı).
...
### Bağımsızlık Sözleşmesi (sigorta — sıra sabit, ama bir paket ertelenirse geçerli)
1. **Semantik sınıf kuralı:** Tüm paketler semantik Tailwind sınıflarını kullanır (`bg-bg-primary`, `accent-primary`, `text-text-muted`…). v1.1 sırasında P1.2 önce geldiği için bu kural artık birincil mekanizma değil **güvenlik ağıdır**: herhangi bir paket ertelenir/atlanırsa kalanlar yine tutarlı çalışır.
2. **Hero ↔ Motion ayrışması:** P1.1 hero'yu VL §07'nin **"bitmiş sahne"** hâliyle (reduced-motion son karesi — VL'ye birebir uygun) statik teslim eder. Koreografi + sinyal tozu P1.4'tedir (v1.1 sırasında P1.1 → P1.4 ardışık — koreografi hedefi hazır olur).
...
**Ön koşullar:** **P1.2 tamamlanmış** (v1.1 sırası — hero final token/font/palet üzerine kurulur; 650 display ağırlığı ve `--light-key`/`--light-fill` hazır olur) + K4 kararı.
...
**Ön koşullar:** **K2 (framer-motion onayı).** v1.1 sırasında P1.1 tamamlanmış olur → koreografi hedefi hazır. (Sigorta: P1.1 ertelenirse koreografi alt-commit'i atlanır, kalanlar bağımsız uygulanır.)
...
| Sıra | Paket | Risk | Ön koşul | Commit | Ana çıktı |
|---|---|---|---|---|---|
| 1 | P1.2 Design system | Yüksek-görsel | **K1 (Bible v1.2)** + K3 | 6 | Token tek-kaynak + palet migration + next/font |
| 2 | P1.1 Hero | Orta | P1.2 + K4 (veri kaynağı) | 4 | Split-hero + SSR + statik VL sahnesi |
| 3 | P1.4 Motion | Orta | **K2 (framer)** | 4 | Reveal/count-up/koreografi/sinyal tozu + hover |
| 4 | P1.3 Dashboard | Orta | — | 4 | 3-saniye hiyerarşisi + AI/risk kartları |
| 5 | P1.5 Mobile & A11y | Düşük-orta | — | 4 | Tablo→kart + Header onarımı + aria + sm |
| 6 | P1.6 Component refactor | Orta (geniş yüzey) | — | 6 | Badge/Modal/toast + ScoreRing + swap/ShareCard |

**Toplam:** ~28 küçük izole commit · **uygulama sırası: P1.2 → P1.1 → P1.4 → P1.3 → P1.5 → P1.6** (v1.1, kullanıcı onaylı) · paket bitmeden sonrakine geçilmez · bağımsızlık sözleşmesi erteleme senaryoları için sigorta olarak korunur.
