...
## Bible v3.0 + Visual-Language v3.0 + Motion-Doctrine v2 + REDESIGN-V3-PLAN — S0 REDESIGN-DOC: «Enstrüman Odası» · 2026-07-18

> **Kapsam:** Ultra-premium redesign v3'ün doc-aşaması (S0). **Doc-only** — kod/CSS/component/UI/backend/DB/scheduler/signal davranışı = SIFIR dokunuş; ürün **byte-identical**. Ana kararlar (kullanıcı-onaylı): (1) **Karot + soyut-3D obje/kristal/orb/brand-object KALICI-DIŞI, gelecek kapı YOK**; VL-MOTION-v3 gated native-3D/video/Spline şeridi **KAPATILDI** — landing derinliği yalnız CSS/Canvas-2D; 3D=gelecekte sıfırdan ayrı karar/sprint. (2) Premium kimlik = **gerçek ürün yüzeyleri + veri/proof kompozisyonu + ışık/derinlik** («Enstrüman Odası», Bible §00-V3: 5 kök karar + anti-pattern DoD-negatif + kopyasız referans-prensipleri). (3) Bible: §02 **hero-v3-kanıt-rasathanesi** · §03 **dash-nabız-v3 Market Pulse** (key-light carve-out; **rejim=VERİ-KAPILI**) + IA-GATE + guardrail-8 · §03-K **v3-sprint-sırası S1–S7** (pv-cp-sırası süpersede; **PV-D3b=Seçenek-A kanon** [bilgi-kaybı=fail, 32px-literal yok]; **K-C2=S4-kapısı**; S5-S6 landing/hero/scroll KİLİTLİ) + RiskFilterBar semantik-iz düzeltmesi. (4) VL: §05 **e3-materyal-v2** (cut-lip+iki-katman gölge+standart scrim+bottom-sheet+kanonik-Modal zorunluluğu) · §03 **efekt-bütçesi-v3** + **BOKEH EMEKLİ** · §09 bayat-satır temizliği (landing count-up "izinli" KALDIRILDI; glow-drift/parallax kapalı-notları) · §01/§02 Karot-emekli/Nabız-carve-out düzeltmeleri. (5) Doctrine v2: sözlük-ekleri (satır-seçimi · Nabız-fotonu · bottom-sheet · scroll-driven) + **S2-BORÇ** sınıfı (sinyal-doğuşu/rozet-crossfade/toast-istifi) + spring resmî-indirgeme + kesintiye-açıklık + route tek-yön + S0 kapı-kayıtları. Kilitler: app'te scroll-jacking=0 · scrollytelling yalnız landing · app-içi tek hero=Nabız · Signal Center=AI Trade Intelligence Center · random-particle yasak. Non-normatif yol haritası: **REDESIGN-V3-PLAN.md** (yeni dosya).
