...
**Version:** v1.3 (KANONIK) · **Tarih:** 2026-07-06
...
- **Teknik:** Fallback bilgi taşıyan katmanı SİLMEZ; yalnız şeffaf/animasyonlu ışık katmanını opak/statiğe çevirir. Karot omurga/düğümler görünür kalır (yalnız glow-underlay kapanır); sayı güncellenir (yalnız flaş kapanır).
...
- **Direktif:** Spinner tamamen kaldır (30 kopya emekli, tam-sayfa ilk-yük istisnası); sinyal loading = Karot'un kendisi; skeleton = boş-Karot (9 düğüm eksende, motor rapor verdikçe yerine kayar). Durum etiketleri GERÇEK pipeline adımları; sahte progress/gecikme yasak; cyan-tarama YOK. <300ms işte jenerik skeleton gösterilmez (grace-window).
- **Token/değer:** grace-window <300ms (jenerik); skeleton = boş-Karot (eksen x=W/2, 9 eşit-aralık düğüm); düğüm-oturma stagger 50ms; süre `--dur-state` (180ms); "durum-etiketli shimmer" YASAK.
...
- **Teknik:** Kontrollü overlay (native `cursor:crosshair` değil); dikey çizgi Karot eksen dili (x=W/2 mantığı). transform/opacity ile takip, layout yok; grafik dışına çıkınca kaybolur.
...
- **Ölçüm:** ilk ekranda ≥40 Karot düğüm-yapısı DOM'da; açılış state'inde split≥1, birth≥1, changed≥1.
...
- **Teknik:** Provenance 140ms giriş (`--dur-warm`), anında çıkış. Bölünmüş sinyalde Karot nötr/zikzak kalır (yön-rengi yok). "AI emin değil" hali de gösterilir.
...
Sinyal kartı ürünün kalbi; merkez enstrüman = **Karot** (moat, Consensus Instrument). Karot 9 motorun yön×güven konsensusunu tek dikey-omurga spline ile görselleştirir. AI-kimliği renkten forma taşınır; Karot yeniden-tasarlanmaz — bilgi-yükü netleşir.
...
- **Token/değer:** primitif `<Karot>` · gösterge tipi dikey-omurga spline (radar/donut/bar DEĞİL).
...
### karot-02 · Geometri + çekirdek konum matematiği
- **Direktif:** Karot'u tek parametrik geometriden türet; hardcode edilmiş görsel şekil yazma.
- **Token/değer:** `W=120 · H=200 · PAD=20 · AXIS=W/2=60 · A(AMP)=AXIS−PAD=40 · yön∈{+1 long,−1 short,0 nötr} · güven∈[0,1] · düğüm=9 · step=(H−2·PAD)/8=20 · viewBox '0 0 120 200'`.
- **Teknik:** `nodes(confs): x=AXIS+c·AMP, y=PAD+i·step` (c işaretli güven ∈[−1,1]). Spline Catmull-Rom→cubic bezier, tanjant (p2−p0)/6 & (p3−p1)/6, uçlarda p0=p1 / p3=p2 klamp. Eksen dashed hairline `--hl12`, stroke-width 0.8, dasharray '2 3', y aralığı PAD−4 → H−PAD+4. Eksen-geçiş sayısı çatışma matematiğinin girdisi (karot-03).
- **DoD:** Tek render fonksiyonu 9-değerli işaretli-güven dizisinden SVG üretir; W/H/PAD/AXIS/A tek kaynakta sabit; hiçbir şekil elle çizilmez.
- **Ölçüm:** unit test: x=60+c·40, y=20+i·20 birebir; spline path deterministik snapshot.
- **Migration (v1.1→v1.3):** v1.1 "renk-şeritli dikey merdiven (ladder)" → parametrik Karot spline; eksen hairline **.16 → .12** (COL-10 hizası).
...
- **Direktif:** 9 motoru omurgada daima bu sırayla (yukarıdan aşağı) diz; motor→düğüm eşlemesi asla değişmez.
- **Token/değer:** sıra (i=0→8): Teknik · Piyasa Yapısı · SMC · CRT · Hacim · Risk · Temel · On-Chain · Makro.
- **Teknik:** ENGINES dizisi sabit tek kaynak; düğüm-i (y=PAD+i·step) daima aynı motor (spatial memory); hover'da motor adı (48px+). Sıra değişmezliği provenance okunabilirliğinin ön koşulu.
- **Ölçüm:** ENGINES tek yerde; indeks→motor eşlemesi unit test.

### karot-05 · Üç dürüst hal + görsel imzaları
- **Direktif:** Üç hali ayrı imzayla render et: Uzlaşma (tek yön + yön tinti dolgusu) / Bölünmüş (cyan omurga, tintsiz, zikzak + eksen-üstü çift-nokta) / Kararsız (soluk, ince, eksene yapışık). Tabloda (16px) YALNIZ 2 hal okunur (uzlaştı/bölündü); Kararsız yalnız 48px+.
- **Token/değer:** Uzlaşma: tint area-fill fill-opacity 0.14, spine stroke-width 1.9, opacity 0.95 · Bölünmüş: stroke `--cyan`, tint=null, çift-nokta imzası · Kararsız: stroke `--tx3`, stroke-width 1.3, opacity 0.6.
- **Teknik:** Uzlaşmada omurga+eksen arası alan yön tintiyle dolar (fill-opacity 0.14). Bölünmüşte renk kazanılmaz (cyan-nötr) + eksen-üstü sabit çift-nokta imzası (bugün render'da YOK → eklenecek). 16px silüette split ile aynı "uzlaşmadı" okuması yeterli.
...
- **Token/değer:** `svg(confs, scale, opt) → {html, cl}`; ölçekler 0.16→~16px silüet (nodes:false), 1→~48px kart, büyük→~200px hero; düğüm r scale≥1→1.9, <1→1.3.
- **Teknik:** `svg()`: classify → nodes → eksen → (consensus) tint area → (consensus && scale≥1) glow underlay → spine → (opt.nodes!==false) düğümler. 16px nodes:false yalnız silüet. Ölçek-değişmezliği (karot-08) bu fonksiyonun garantisi.
...
- **Direktif:** Aynı Karot'u üç ölçekte tek okuma diliyle render et: 16px silüet (tablo — yön+uzlaşma) / 48px kart (motor adları hover) / 200px hero-vitrin (etiketli, canlı settle, provenance). 200px hero dâhil TÜM ölçekler 2D SVG — WebGL/3D DEĞİL.
- **Token/değer:** 16px silüet nodes:false glow-yok · 48px hover motor adları · 200px etiket+settle+provenance · malzeme 2D SVG (WebGL yasak).
- **Teknik:** Küçük ölçekte düğüm/glow düşer (nodes:false, glow yalnız scale≥1). 200px hero-vitrin dekoratif kamera/3D olmadan yalnız ışık+settle ile sinematiktir. Ölçek büyüdükçe bilgi eklenir, okuma dili değişmez.
...
- **Token/değer — kanonik rest-state confs dizisi (i=0→8, Teknik→Makro), c∈[−1,1]:**
  ...
  - Türetilen metrikler: `mean = +0.364`, `absMean = 0.364` (≥0.30 → Kararsız DEĞİL), işaret bandı sonrası tüm düğümler +1 (|c|>0.08), eksen-geçiş `cross = 0` (<2 → Bölünmüş DEĞİL) ⇒ hal = **Uzlaşma**, yön = **bull** (mean>0). İmza: hafif-bull-uzlaşma (omurga eksenin sağında toplanır, aşırı bükülme yok — sakin/dürüst konsensus silüeti).
...
- **Token/değer:** spine stroke-width uzlaşma/bölünmüş 1.9, kararsız 1.3 · linecap/linejoin round · uzlaşma glow underlay (yalnız scale≥1): stroke-width 3.4, opacity 0.14, blur 2px · radius yok.
- **Teknik:** İnce net vektör omurga (1.3–1.9px). Uzlaşmada scale≥1'de düşük-opaklık blur underlay = **cyan-omurga çizgisinin AYNI-çizgi ışık-genişletmesi** (stroke-width 3.4 = omurga stroke kalınlaştırılmış kopyası), bağımsız dolgu-yüzey DEĞİL → cyan-yüzey tabusuna girmez; statik 2px blur backdrop değil enstrüman-blur (blur-yalnız-E3 lint'inden muaf), opaklık ≤.14 tavanına tabi. 16px yoğunluk yüzeyinde glow KAPALI. Radyussuzluk Karot'u jenerik radius-12 kartlardan ayıran imza.
...
- **Token/değer:** donut/bar rolü = toplam/skaler özet (ör. "82/100") · Karot rolü = per-motor ayrışma (9 düğüm) + süreç/zaman temsili · 48/200px ölçekte etkileşimde per-motor gerekçe açılır.
...
- **Direktif:** Sinyal üretiminde loading Karot'un kendisi: 9 düğüm eksende başlar, motor rapor verdikçe yerine kayar. Skeleton = boş-Karot. Spinner, cyan-tarama beat'i, "durum-etiketli shimmer" kullanılmaz.
- **Token/değer:** başlangıç 9 düğüm eksende (x=AXIS, kararsız hal) · düğüm çözülme stagger 50ms (izinli TEK stagger) · düğüm easing `--dur-state` (180ms, `--ease-signal`) · cyan-tarama = YASAK.
- **Teknik:** Düğümler eksenden başlar; motor-önceliği sırasıyla (yukarıdan aşağı) yerine kayar. Kaynak director-cut'taki "tek cyan tarama" beat'i KALDIRILDI (cyan-tarama AI-tropu ebedî yasak). Sinyal-Karot skeleton (boş-Karot = üretim başladı) grace'ten MUAF, her zaman gösterilir.
- **Ölçüm:** sinyal-loading'de spinner=0; cyan-tarama pattern=0; skeleton=boş-Karot render; düğüm-çözülme süresi `--dur-state`.
...
- **Token/değer:** no-birth koşulu eksen-geçiş≥2 OR absMean<0.30 → Bölünmüş (doğum yok); bölünmüş imza cyan omurga + tintsiz + zikzak + eksen-üstü çift-nokta; ilk-ekran ≥1 bölünmüş.
...
| `--dur-state` | `180ms` | Aç/kapa/geçiş (dropdown/tooltip/panel state); Karot düğüm-çözülme bu banda oturur (`--ease-signal`) |
...
- **Direktif:** Son motor çözülünce omurga TEK yay hareketiyle otursun; düğüm çözülüşü 50ms stagger, omurga settle spring; bir kez oynat, ASLA geri sarma.
- **Token/değer:** düğüm-stagger 50ms + `--ease-signal` · omurga-settle spring `--dur-settle` (520ms, ≤600ms) · uzlaşma → yön tinti akışı + kalıcı cyan kenar (rim).
- **Teknik:** (1) 9 düğüm eksenden konumlarına 50ms stagger (`--ease-signal`); (2) omurga tek yay (spring, `--dur-settle`). Uzlaşmada yön tinti gövdeye akar + kalıcı cyan rim; bölünmüş/kararsızda renk yok. Tek atım; sonra statik, geri sarılmaz.
...
- **Ölçüm:** settle ≤600ms; düğüm stagger 50±10ms; `animation-iteration-count=1`; reduced-motion'da statik son-kare (doğum posteri); uzlaşma→rim var, bölünmüş→rim yok; overshoot ölçülen ≤%8, settle-tolerans ≤0.5px.
...
- **Ölçüm:** liste-giriş gecikmeleri 50ms bir kez (loop yok); Karot düğüm stagger 50±10ms; app-route stagger yalnız `{liste-giriş, Karot-oy}`.
