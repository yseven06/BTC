...
**Version:** v3.0 (KANONIK) · **Tarih:** 2026-07-18 · *(v3.0 S0 REDESIGN-DOC: **«Enstrüman Odası» redesign-v3 kanonu** — YENİ §00-V3 [5 kök karar · **Karot + soyut-3D obje/kristal/orb/brand-object KALICI-DIŞI, gelecek kapı YOK; VL gated native-3D/video/Spline şeridi KAPATILDI** · anti-pattern DoD-negatif listesi · kopyasız referans-prensipleri] · §02 **hero-v3-kanıt-rasathanesi** (katmanlı proof-surface, yalnız CSS/Canvas-2D) · §03 **dash-nabız-v3 Market Pulse** (key-light carve-out kullanımı; rejim=VERİ-KAPILI) + IA-GATE + guardrail-8 güncellendi · §03-K **v3-sprint-sırası S1–S7** (pv-cp-sırası süpersede; PV-D3b=Seçenek-A kanon; K-C2=S4-karar-kapısı) + RiskFilterBar semantik-iz düzeltmesi. VL v3.0 + MOTION-DOCTRINE v2 + REDESIGN-V3-PLAN.md aynı revizyonda senkron. **Doc-only — runtime davranışı byte-identical.** bkz CHANGELOG.)* · *(v2.0 CP-PREMIUM-VISUAL-A: **Premium Yüzey Sahneleme — YENİ §03-K** [Dashboard durum-odası + Signal Center cockpit sahneleme reçeteleri · kap-ataması chrome/panel/well · kahraman-well sahneleri · **Karot'suz hairline-iskelet loading dili** · GlassCard→E-merdiveni netleştirmesi (blur yalnız E3) · uygulama sırası PV-B/C/D→CP-MOTION-A→CP-LANDING-3D-A] + **bayat-Karot re-anchor** [G-00-14 imza-listesi banner · Button-loading satırı · micro-loading-boş-karot SUPERSEDED · §01-K kimlik-üçlüsü]. IA değişmedi (sorumluluk-tablosu kilitli) · Karot geri GELMEDİ · yeni token/renk/süre/glyph YOK · app motion sınırı korunur (3D/ambient/cinematic yalnız landing-gated). VL v2.2 + MOTION-DOCTRINE aynı revizyonda senkron. bkz §03-K + CHANGELOG.)* · *(v1.9 CP-1 VL-MOTION-v3 · VL v2.1: **native-motion taksonomisi + gated 3D/video/Spline politikası** — generic/stok/template/sahte/perf-riskli yasak AYNEN; TradeMinds-native × bilgi-taşıyan 3D/video/motion = **koşullu-serbest gated-future-CP** (8-kabul-kapısı); video-bg + Spline düz-yasağı + native-3D DEFERRED → gated-conditional; §00 G-00-15 + §02 hero-3d-koşullu + Hero-Kalıcı-Yasaklar senkron; app ağır-motion yasağı + Karot-geri-gelmez + yeni-glyph-yok korunur; kanonik ayrım = VL "VL-MOTION-v3" bölümü. bkz CHANGELOG.)* · *(v1.8 CP-HERO-DOC · VL v2: **landing-hero doktrini Karot'suz REVİZE** — §02 hero-merkezi "40 Karot canlı konsensüs masası" + Karot-squint SUPERSEDED (Karot kaldırıldı, [[karot-full-removal]] / CP-KAROT-DOC v1.7). Yeni tez = Canlı-Masa / live-terminal proof-surface + owned-number hero + proof-receipt + instrument-well (glyph/ikon/şekil İKAMESİ YOK); motion reveal-only; alt-CP DOC→B→C→A. bkz §02 baş-banner + VISUAL-LANGUAGE v2.0 + CHANGELOG.)* · *(v1.7 CP-KAROT-DOC: **Karot ürün-UI DEPRECATION** — Karot artık üründe zorunlu kimlik / merkez-enstrüman / marka-işareti DEĞİL; §05 [karot-01…15 + sahne-ölçekleri] ürün-UI için SUPERSEDED (tarihsel kayıt olarak korundu). Yeni kimlik = kompozisyon / typography / owned-numeral / instrument-well / proof-receipt / dark-terminal (glyph ikamesi YOK; §05 baş-banner + §1124). UI kaldırma CP-KAROT-UI1/2/3, ölü-kod CP-KAROT-CLEAN. bkz CHANGELOG.)* · *(v1.6 CP-PV1-A: Kompozisyon & Sahneleme katmanı — kap-hiyerarşisi [chrome/panel/well] · veri-kuyusu · kahraman-rakam · makbuz-grameri · sessiz-buton · Karot sahne-ölçekleri · referans-politikası; bkz. §01 sonu + §05 karot-sahne-ölçekleri + CHANGELOG. v1.5 CP-PIA: bilgi-mimarisi kilidi [Durum Odası · İstihbarat Merkezi · widget-taşıma · sorumluluk-tablosu · IA-guardrail], bkz. §02/§03/§04. v1.4: Karot geometri revizyonu + Geometry Freeze, bkz. §05 karot-02/15)*
...
## 00-V3 · REDESIGN v3 — «Enstrüman Odası» (S0 KANONU · 2026-07-18)

> **Ana karar (bağlayıcı, kullanıcı-onaylı):** TradeMinds'in tüm görsel deneyimi tek premium sistem olarak v3'e taşınır: Landing Hero + Scrollytelling + Dashboard Nabız + panel sistemi + Signal Center Cockpit + Popup/Modal/Drawer + Motion + Visual Effects **bütündür**, parça-düzeltmesi değildir. Uygulama sırası = §03-K **v3-sprint-sırası** (S1–S7; landing/hero/scroll S5-S6 KİLİTLİ — plan panel/table düzeltmesine indirgenemez). Normatif kurallar bu üç kanonik belgededir; yol haritası ayrıntısı [REDESIGN-V3-PLAN](./REDESIGN-V3-PLAN.md) (non-normatif).

### v3-yön · «Enstrüman Odası»
Dışarısı rasathane (landing: atmosfer · ışık · anlatı), içerisi hassas alet (app: steril · yoğun · anında). Ton: dark · masculine · luxury-fintech (Apple/Linear/Stripe craft × Bloomberg/TradingView/Hyperliquid veri-ciddiyeti). Beş kök karar:
1. **Zemin:** near-black soğuk-cast + algısal-eşit (OKLCH) E-merdiveni — derinlik gölgeden değil kalibre basamaktan.
2. **Renk = olay:** bull/bear yalnız ŞU AN olan durumda yaşar; kalıcı dekoratif renk = yalan söyleyen sinyal.
3. **Işık = veri (diegetik):** her glow/parıltının makbuzda karşılığı olan gerçek bir olayı vardır; app'te ambient = 0.
4. **Motion = olay-sözlüğü:** Beş Yasa sürer (Motion Doctrine v2); sözlükte karşılığı olmayan hareket otomatik red.
5. **Kimlik = kompozisyon:** typography · kahraman-rakam · instrument-well · proof-receipt · hairline · dark-terminal. **Glyph/maskot/marka-objesi YOK.**

### v3-karot-kapanış · Karot + soyut-3D KALICI-DIŞI (gelecek kapı YOK)
Kullanıcı kararı (S0): Karot **hiçbir yüzeyde** (hero/marka/ürün/creative-asset) kullanılmaz ve **gelecek karar-kapısı olarak da tutulmaz**; asset planlamasında Karot ayrı marka-asset'i olarak YER ALMAZ. "Soyut 3D obje · kristal · orb · brand-object · hero-object · generic-3D-hero" sınıfı tümüyle bu kapsamdadır. **VL-MOTION-v3'ün gated native-3D/video/Spline şeridi de S0'da KAPATILDI** (bkz. VL "VL-MOTION-v3" S0-banner'ı) — landing derinliği yalnız **CSS/Canvas-2D** (CP-HERO-PROOF-SURFACE ailesi). 3D ancak gelecekte **sıfırdan, ayrı karar ve ayrı sprint** olarak açılabilir; bu redesign planının parçası değildir. Premium kimlik yalnız **gerçek ürün yüzeyleri + veri/proof kompozisyonu + ışık/derinlik**ten kurulur (gerçek dashboard yüzeyleri · cockpit · proof-surface/receipt · data-panels · key-light · depth · hairline-grid · controlled-glow · premium overlay sistemi · landing scroll-reveal · Nabız · gerçek-veri hissi).

### v3-anti-pattern · Yasaklı görsel dil (DoD-negatif · release-red)
neon-casino (doygun neon · kenar-glow kart · idle yanıp-sönme) · oyuncak-SaaS (pastel gradient · emoji-ikon · konfeti · maskot) · **random-particle/yıldız-tozu arkaplanı** · robot/beyin/globe/**orb/kristal** klişesi · jenerik glassmorphism (blur-kart; E3 dar-reçetesi tek işlevsel istisna) · AI-slop kalıp seti (mor-gradient hero · H1-üstü badge · renkli-üst-kenarlı kart · özdeş ikon-kart grid'i · her yere gradient) · saf-#000/saf-#FFF uçları · her-şeye-glow ("her şey parlarsa hiçbir şey parlamaz") · scroll-hijack / ürün-içi sinema · dekoratif/idle animasyon · **sahte-canlı veri** · dokunulmamış-default component görünümü · yoğunluk-korkusu (boşluk şişirme) · hex-kopyacılığı.

### v3-referans-prensipleri (kopyasız; S0 referans-taraması)
Disiplin>dekor (tek aksan; hiyerarşi = basamak+hairline) · LCH/OKLCH-eşit elevation · hairline>gölge (gölge yalnız E3, iki-katman) · tabular **"sayı asla zıplamaz"** sözleşmesi · **6-mikro-durum tamlığı** (default/hover/focus/active/disabled/loading — premium'un ölçülebilir tanımı) · Bloomberg sıfır-gecikme (**"veri T0'da okunur; motion sonra süsler"**) · diegetik ışık · grain yalnız landing-statik-≤%3 ("atmosfer dışarıda, hassasiyet içeride") · scrollytelling = anlatı-kavisi · hero'da ürünün kendisi · görünmez-craft (optik hiza) · semantik-önce sözlük (~5 kök karar + türetme; "değerler çalınabilir, sözlük çalınamaz").

### v3-scope-dışı (değişmez)
Gerçek veri semantiği · signal generation/scoring/risk/outcome/tracking/lifecycle logic · backend/API/DB/scheduler · legal/disclaimer konumlandırması ("yatırım tavsiyesi vermeme" çizgisi) · filtre/sıralama/persist davranışı. S0 = yalnız dokümantasyon; runtime davranışı byte-identical.
...
> **⚠ v2.0 RE-ANCHOR (CP-PREMIUM-VISUAL-A):** Bu listenin **Karot-bağımlı ögeleri (Karot-merkez · doğum-nabzı · kanıt-fitili) ürün-UI için SUPERSEDED** (Karot kaldırıldı, v1.7). Güncel kimlik-taşıyıcı seti = **§1124 revizyonu**: kabin-imzası · instrument-well · proof-receipt · owned-palet · tabular-numeral (+ közleşme · lümen-ölçek · ışık-maskeli-grid korunur). "Karot zorunlu" DoD/Ölçüm satırları normatif DEĞİL → yeni kapı: VL DoD-2 (v2.2 kompozisyon-zorunluluğu) + §03-K DoD. Aşağıdaki orijinal liste tarihsel.
...
> **⚠ VL-MOTION-v3 REVİZE (v1.9):** Bu liste **generic/kimlik-merkezi/dekoratif/sahte** kullanım için AYNEN geçerli (release-blokaj). **Tek istisna:** *TradeMinds-native × bilgi-taşıyan* **video / Spline / native-3D/WebGL** (ürün proof-surface / instrument-well derinliği) = **gated-conditional future CP** — 8-kabul-kapısı + ayrı onay (kanonik: VL "VL-MOTION-v3" bölümü). Generic globe/orb/robot/avatar/template/neon + yeni-glyph icadı + Karot-dönüşü AYNEN yasak; app-yüzeyi (Dashboard/Signal Center) ağır-motion yasağı korunur.
...
- **Token/değer:**
  ...
  | loading | metin→inline **hairline-iskelet** (v2.0: boş-Karot emekli — §03-K pv-yükleme); spinner-yok; genişlik sabit (layout kaymaz) |
...
> **⚠ SUPERSEDED (v2.0 CP-PREMIUM-VISUAL-A):** Karot ürün-UI'dan kaldırıldı (v1.7) → **boş-Karot skeleton artık normatif DEĞİL.** Kanonik loading dili = **§03-K `pv-yükleme-hairline-iskelet`** (statik hairline placeholder; spinner-yasağı + <300ms grace + sahte-progress-yasağı + gerçek-pipeline-etiketi kuralları oradan AYNEN devralınır). Aşağıdaki metin tarihsel kayıt.
...
- **Direktif:** Her ana ekranın TEK kahraman-rakam hakkı vardır (40–56px, `tabular-nums`, owned-numeral): Dashboard-Sicil = **Dönem Net Getiri** · Signal-Center-Dock = **konsensus-skoru**. Altında zorunlu makbuz-satırı (craft-makbuz-grameri). G-00-02 kimlik-taşıyıcı üçlüsünün (Karot · provenance · kahraman-rakam) üçüncüsü İLK KEZ sahnelenir. *(v2.0 re-anchor: Karot kaldırıldı — taşıyıcı üçlü artık **provenance-makbuz · kahraman-rakam · instrument-well**; sahneleme §03-K.)*
...
### hero-v3-kanıt-rasathanesi · Landing Hero v3 — «Kanıt Rasathanesi» *(S0 v3.0 · uygulama S5)*
- **Direktif:** Hero, Karot'suz tezin (v1.8) DERİNLEŞTİRİLMİŞ halidir: merkez = gerçek Canlı Masa'nın **katmanlı proof-surface sahnesi** — masa yüzeyi (E0-kuyu) + gerçek kayıt-satırları + üstte gerçek foton-olayları; derinlik **yalnız CSS/Canvas-2D** katman-ayrımı + ışık farkından (parallax yok). `--light-key` masaya "düşer" (kaynağı ve hedefi belli tek ışık). H1 statik/LCP kilidi + T3 reveal + grain AYNEN.
- **Sinyal-tozu (VL §02 L4):** Canvas-2D, YALNIZ landing-hero, **diegetik** — her nokta gerçek-olay-havuzundan; random-particle DEĞİLDİR. rAF + IO-görünürlük-pause + parçacık-tavanı + DPR-cap + reduce'ta hiç kurulmaz.
- **Scroll-reveal prensibi:** CSS `animation-timeline: scroll()/view()` + IO-fallback; tek-atım; pinning/scrub/Lenis/wheel-intercept yasak (S6 anlatı-kavisi: *sinyal doğar → yaşar → makbuz kesilir → sicil birikir*).
- **Mobil fallback:** tek-kolon; toz kapalı/düşük-yoğunluk; scroll-driven → basit IO `rv-in`; CLS=0; LCP bütçesi mobil-öncelikli.
- **Yasak (S0 kalıcı):** Karot · soyut-3D obje · kristal/orb · brand/hero-object · generic-3D hero · **native-3D/video/Spline gated şeridi KAPALI** (v3-karot-kapanış + VL S0-banner) · video-bg · mock/scripted-timeline.
- **DoD:** hero'daki her derinlik/ışık/hareket ögesi gerçek veri/olaya iz-sürülebilir; squint = {H1, birincil-CTA}; LCP elemanı statik; 3D/WebGL importu = 0.
...
### hero-3d-webgl-koşullu · *gated-conditional future CP (VL-MOTION-v3, v1.9)*
> **⚠ VL-MOTION-v3 REVİZE:** "açık uç / DEFERRED / ŞU AN 0" → **gated-conditional future CP**. Generic/dekoratif 3D yasak AYNEN; yalnız **TradeMinds-native × bilgi-taşıyan** proof / instrument-well derinliği, **8-kabul-kapısı** + mobil-fallback + reduced-motion-statik + LCP/CLS-koruması + ayrı onayla açılabilir (CP-HERO-NATIVE-3D). Kanonik ayrım: VL "VL-MOTION-v3" bölümü.
...
> **⚠ VL-MOTION-v3 (v1.9):** "Sahte-sinema" (generic / dekoratif / scripted) yasağı AYNEN. Ayrım netleşir: *native × bilgi-taşıyan* proof-surface motion / 3D / video ≠ sahte-sinema → **gated-conditional future CP** (8-kabul-kapısı; VL "VL-MOTION-v3"). Robot/globe/orb/template/neon + yeni-glyph AYNEN yasak; 3D/video yalnız landing (app-yüzeyi ağır-motion yasağı korunur).
...
### dash-nabız-v3 · Market Pulse *(S0 v3.0 · uygulama S3)*
- **Direktif:** Nabız, app'in TEK hero'su olarak "executive market pulse" seviyesine çıkar — piksel değil **bilgi + ışık** büyür: (a) **key-light carve-out'u nihayet KULLANILIR** — banda statik `--light-key` katmanı (z-altı, alfa ~.05, ΔL* ≤ +6, scroll'suz, ambient-animasyonsuz); `--light-fill`/cyan-yüzey KULLANILMAZ (gate-3 tekeli). (b) **Olay-bağlı tek-atım:** refresh'te `closedCount`/`activeCount` GÖRÜNÜR değişince mevcut foton/flash sözlüğüyle tek-atım; idle'da sıfır hareket. (c) **Sistem-sesi zenginleşir (mevcut veriyle, backend'siz):** lifecycle-census ("N TP'ye yaklaşıyor · M zayıflıyor") + "bugün doğan N" (`generated_at` client-türetme) + LONG/SHORT + ort-güven.
- **Rejim = VERİ-KAPILI:** dash-nabız-bandı tanımındaki "rejim" bileşeni, frontend'e açık güncel-rejim endpoint'i OLMADIĞI için dürüstçe OMIT edilir (per-sinyal intelligence yalnız doğum-rejimi taşır ≠ güncel rejim; fan-out = pool-riski). Sahnelenmesi = ayrı-onaylı read-only backend micro-endpoint CP'si; o güne dek banda "rejim" YAZILMAZ (mock=fail).
- **Sınırlar (AYNEN):** ≤~120px · kap'sız chrome-üstü · pazarlama-hero yasak · F&G piyasa-bağlamı satırında kalır (taşıma-kilidi) · squint'te en parlak 2 = {kahraman-rakam, birincil-CTA} — **Nabız üçüncü parlak bölge OLUŞTURMAZ** (ışık loş). Karot/soyut-3D: yok (v3-karot-kapanış).
...
> **IA-GATE (S0 v3.0 karar-kapısı · S3/S4'te sunulur):** AI-Görüşü (Nabız'a fold) ve Risk-Dağılımı (Signal Center enstrümanı) yerleşimi v3'te **varsayılan olarak KORUNUR**; S3 (Dashboard Pulse) ve S4 (Cockpit) kapsam-analizlerinde bu tablo yeniden sunulur — kullanıcı onayı olmadan geri-taşıma yapılmaz.
...
8. *(v3.0 güncellendi)* Karot **HER yüzeyde** yasak — ürün-UI + marka + creative-asset (CP-KAROT-DOC v1.7 + §00-V3 v3-karot-kapanış: gelecek kapı YOK); glyph/maskot/soyut-3D ikamesi yasak.
...
## 03-K · Premium Yüzey Sahneleme (Premium Surface Staging) — Dashboard + Signal Center · *(v2.0 CP-PREMIUM-VISUAL-A)*

> **Statü:** doc-only karar kilidi (§01-K / CP-HERO-DOC deseni). Uygulama = **CP-PREMIUM-VISUAL-B/C/D** (her biri ayrı onay + görsel-QA). **Bu bölüm yeni tasarım İCADI değildir:** kilitli token seti (§01) + kompozisyon craft'ları (§01-K) + IA kilitleri (§03 sorumluluk-tablosu · CP-DASH-IA-A bridge-glance) ÜZERİNE iki ana app-yüzeyinin premium sahneleme reçetesini kilitler. **IA DEĞİŞMEZ · Karot geri GELMEZ · yeni token/renk/süre/glyph İCAT EDİLMEZ · app motion sınırı korunur** (yalnız T0–T2 mikro; 3D/bokeh/ambient/cinematic = landing-gated, VL "VL-MOTION-v3" + Doctrine).

### pv-kapsam · UYGULANACAK / UYGULANMAYACAK

| UYGULANACAK | UYGULANMAYACAK |
|---|---|
| Kap-hiyerarşisinin (chrome/panel/well) iki yüzeye blok-bazında ataması | Yeni token/renk/süre/glyph/ikon icadı |
| Owned-numeral kahraman sahneleri (well içinde) + zorunlu makbuz | Karot'un veya herhangi bir dekoratif sembolün dönüşü |
| Karot'suz loading dili (**hairline-iskelet**) | IA/sorumluluk değişikliği (§03 tablo kilitli) |
| Sessiz-buton + mevcut badge envanterinin uygulanması | App'te 3D/bokeh/ambient/cinematic/scroll-motion |
| GlassCard→E-merdiveni netleştirmesi (blur yalnız E3) | Landing motion/3D üretimi (ayrı gated ark: CP-LANDING-3D-A) |
| Tablo/satır/receipt sistemi standardizasyonu (PV-D) | Backend/API/DB/sinyal-davranışı; Higgsfield/asset promptu |

### pv-kap-ataması · Kap-hiyerarşisi yüzey ataması (craft-kap-hiyerarşisi uygulaması)

- **chrome** (nav · başlık-satırı · dönem-toggle · filtre-barı iskeleti): cömert 24px nefes, hairline `--hl12`, zemin `--e0/--e1`; vurgu taşımaz.
- **panel** (`--e1` + `--cut-lip`, gölgesiz — craft-card): stat-tile'lar · Sicil grubu · Dock gövdesi · tablo kabı.
- **well** (`--e0` + `inset 0 0 0 1px var(--hl10)` — craft-veri-kuyusu): **YALNIZ kahraman-veri sahneleri** — Dashboard kahraman-rakamı ve Signal-Center Dock kahraman-sahnesi. **Ekran başına well ≤2**; well içinde dekor=0 (mevcut ölçüm AYNEN).
- **Blur/glass netleştirmesi:** `backdrop-blur` YALNIZ **E3 overlay** (modal/drawer/toast — VL §05). Yüzey kartlarında blur=0 → mevcut `GlassCard` kullanımı **lüminans-kartı** olarak netleşir (E1 + cut-lip; blur'lu varyant yalnız overlay bağlamında). Uygulama/adlandırma = PV-B işi; bu satır KARARDIR.

### pv-dashboard-reçete · Dashboard sahneleme (Durum Odası)

IA §03'te kilitli; aşağıdaki satırlar YALNIZ görsel sahnelemedir:

| Blok | Kap | Reçete |
|---|---|---|
| Nabız (`DurumBandi`) | chrome-üstü yaşayan başlık | ≤~120px; atmosfer-ışığı app'te YALNIZ burada (dash-nabız-bandı carve-out); motion=olay-bağlı tek-atım; ambient=0 |
| Kahraman: **Dönem Net Getiri** | **well** | 40–56px `num font-num-560 tabular-nums`; altında zorunlu makbuz (`n işlem · dönem · %win`); count-up ASLA; ekranın EN PARLAK ögesi |
| 3 ikincil (Aktif · Kapanan · Başarı) | panel (dense) | sessiz, eşit; kahramanla vurgu yarıştırmaz; mikro-etiket ≥`--tx2` |
| Aktif Sinyaller bridge-glance (`ActiveSignalGlance`) | hafif-panel (hairline liste) | top-4 salt-okur (CP-DASH-IA-A kilidi); satır = sembol + yön + `LiveStatusBadge` + tabular fiyat; hover satır-ısınma E1→E2 `--dur-warm` **transform 0**; yoğunluk-toggle YOK (Signal Center'ın) |
| Sicil | panel | makbuz-grameri başlıklar; közleşme kuralları (§01) — kapanan kayıplar görünür |
| Piyasa-bağlam satırı | chrome | tek kompakt satır (mktcapΔ · BTC-dom · F&G); gauge/pie/dekoratif-svg=0 |

- **Squint (Dashboard):** en parlak 2 = **{kahraman-rakam, birincil-CTA}**; vurgu-bütçesi ≤2 (dash-vurgu-bütçesi) AYNEN.

### pv-signal-center-reçete · Signal Center sahneleme (İstihbarat Cockpit'i)

| Blok | Kap | Reçete |
|---|---|---|
| `SignalTable` | panel içinde yoğun-veri | 13px gövde tabular · `--row-h` 32/40 · satır-ısınma +1 lümen **transform 0** · güncelleme=foton/flash olay-sözlüğü (Doctrine §3); "veri yoğun, chrome sakin" (dash-yoğunluk-sözleşmesi) |
| `RiskFilterBar` | chrome-enstrüman | dilime-tık=filtre (guardrail-5: dekor OLAMAZ); aktif segment izi risk-SEMANTİK — low=bullish · medium=amber · high=bearish (RiskDagilimi tutarlılığı; *v3.0 düzeltme: eski `--accent-ui` ibaresi koddaki semantik gerçeğe hizalandı*) |
| Dock (kalıcı kanıt paneli) | panel gövde + **well kahraman-sahnesi** | Dock kahraman-rakamı = **konsensus-skoru** (craft-kahraman-rakam kilidi) well içinde + makbuz `9 motor · 7 LONG · 2 nötr`; **glyph=0 — kimlik SAYI + makbuzdan okunur** |
| SignalDetail / motor-skorları | panel | metin/bar dili (Karot'suz mevcut hal); AT-2 provenance-hover makbuz-formunda; owned-numeral yalnız Dock-kahramanda (ekran-başına-1 kuralı) |
| Empty / Loading | panel | `EmptyState` primitifi (micro-empty-olay-defteri) · **hairline-iskelet** (aşağıda) |

- **Squint (Signal Center):** en parlak 2 = **{Dock kahraman-rakamı, birincil-CTA (varsa)}**; tablo yüzeyinde glow=0.

### pv-yükleme-hairline-iskelet · Loading dili (Karot'suz KANONİK) — micro-loading-boş-karot'un halefi

- **Direktif:** Yükleme = **hairline-iskelet**: nihai içerik-bloklarının SABİT-BOYUT, **STATİK** placeholder'ları (nefes/pulse/shimmer YOK — "emin olan enstrüman titremez", Doctrine Yasa-3). Spinner-yasağı (tam-sayfa ilk-yük istisnası) + **<300ms grace** + sahte-progress-yasağı + gerçek-pipeline-etiketi kuralları micro-loading-boş-karot'tan **AYNEN devralınır**; yalnız Karot-formu emeklidir.
- **Token/değer:** placeholder zemini mevcut düşük-alfa banttan (`--hl10` hairline-ailesi hedef-tek-aile; mevcut `bg-white/[0.06]` kullanımları PV-B/C'de bu banda hizalanır — **yeni token AÇILMAZ**); blok boyutları nihai içerikle birebir (**CLS=0**); stagger 50ms yalnız liste-iskelet sırasında.
- **DoD/Ölçüm:** inline spinner=0 (tam-sayfa hariç) · animasyonlu skeleton (pulse/shimmer)=0 · <300ms işte iskelet yanmaz · iskelet-blok↔içerik boyut farkı=0 (CLS) · `<Karot`=0.

### pv-motion-sınırı · Motion sınırı (app — DEĞİŞMEDİ, satır-satır bağlanır)

App yüzeyleri yalnız **T0–T2 mikro**: hover-ısınma `--dur-warm 140` · state `--dur-state 180` · foton `--dur-photon 150` / flash `--dur-flash 300` (dar-tanım) · route `--dur-route 180` · overlay `--dur-overlay 360` (Doctrine §1–§3). **Scroll-tetikli/parallax/ambient/3D/bokeh/cinematic app'te YOK** (dash-ambient-yasak + VL-MOTION-v3 kapı-7). Landing motion/3D **ayrı gated ark** (CP-LANDING-3D-A; VL "VL-MOTION-v3" + §02).

### pv-bayat-karot-reanchor · Bayat-Karot re-anchor haritası (bu revizyonda kapatıldı)

| Madde | Eski (bayat) | Yeni (v2.0/v2.2) |
|---|---|---|
| §01 micro-loading-boş-karot | skeleton = boş-Karot | SUPERSEDED → **pv-yükleme-hairline-iskelet** |
| §01 Button loading state | inline boş-Karot mikro-skeleton | inline **hairline-iskelet** |
| §00 G-00-14 imza listesi | Karot-merkezli 8 imza + "Karot zorunlu" DoD | banner: taşıyıcılar = §1124 seti; kapı → VL DoD-2 (v2.2) |
| §01-K kimlik-üçlüsü | (Karot · provenance · kahraman) | **provenance-makbuz · kahraman-rakam · instrument-well** |
| VL DoD-2 | "her sinyal yüzeyinde Karot" | **kompozisyon-zorunluluğu** (taşıyıcı ≥1 + glyph=0) |
| VL DoD-4 app-squint | {Karot, CTA} | **{kahraman-rakam, birincil-CTA}** |
| VL §09 skeleton/stagger/settle notları | sinyal-Karot skeleton / fitil-dönüşü / Karot-settle | hairline-iskelet / emekli-işaret |
| Doctrine §3 "Yükleme" satırı | Boş-Karot statik | **Hairline-iskelet statik** |

### pv-cp-sırası · Uygulama CP sırası (kilit)

1. **CP-PREMIUM-VISUAL-B** — Dashboard visual surface implementation (kahraman-well sahnesi · GlassCard/E-merdiveni netleştirme · glance mini-polish bundle).
2. **CP-PREMIUM-VISUAL-C** — Signal Center visual surface implementation (Dock kahraman-well · tablo-chrome ayrımı · Signal-Center minör IA-cleanup bundle).
3. **CP-PREMIUM-VISUAL-D** — table/row/receipt system (satır-states · makbuz-standardizasyonu · hairline-iskelet rollout).
4. **CP-MOTION-A** — app micro-motion rules (Doctrine'e tam-uyum denetimi + kalan `transition-all` borçları).
5. **CP-LANDING-3D-A** — landing 3D/motion asset direction (VL-MOTION-v3 8-kabul-kapısı; **gated**; app'e taşmaz).

Her biri: salt-okuma analiz → onay → küçük commit → **gerçek görsel-QA (desktop+mobil)** → ayrı push onayı.

> **⚠ SUPERSEDED (v3.0 S0):** Yukarıdaki pv-cp-sırası tarihsel kayıttır — PV-B/PV-C/PV-D1/D2/D3a commit'lenmiştir; kalan işler aşağıdaki **v3-sprint-sırası**na devredildi.

### v3-sprint-sırası · REDESIGN v3 uygulama sırası *(S0 KİLİT · pv-cp-sırasını süpersede eder)*

1. **S1 · Material & Overlay Foundation** — **e3-materyal-v2** (VL §05): cut-lip'in cam-reçetesine eklenmesi + iki-katman `--shadow-e3` + standart scrim (`bg-e-0/70`+blur-sm, senkron fade) + 4 ad-hoc modal kabuğunun (SignalDrawer · ClosedSignalChart · EngineDetail · ShareCard) kanonik `ui/Modal`'a göçü + **PI-1b** (Dropdown/Tooltip reverse-exit) + **mobil bottom-sheet** (`align="bottom"`+slideUp+drag-handle) + z-token göçü + Tooltip materyal-yükseltme + OKLCH basamak-kalibrasyon denetimi.
2. **S2 · Motion-Core** *(≈eski CP-MOTION-A)* — 32 `animate-spin` spinner borcunun hairline-iskelete göçü + landing `transition-all` borcu + sözlük-borçları implement (**sinyal-doğuşu · rozet-crossfade · toast-istifi**) + row-selection tek-atım + spring-spec resmî indirgeme (Doctrine v2) + hairline-grid empty-state craft'ı.
3. **S3 · Dashboard Pulse** — **dash-nabız-v3** (key-light + olay-fotonu + zengin sistem-sesi) + Sicil tam-makbuz + ölü `AIGorusu`/`RiskDagilimi` CLEAN-mini + **IA-GATE** sunumu.
4. **S4 · Signal Center Cockpit** — **PV-D4** receipt-recompose (seviye-well · on-chain-well · LONG/SHORT nötr-yüzey · census `ProvenanceReceipt` tek-biçim) + **PV-D3b = Seçenek-A KANONU:** yoğunluk yalnız padding-trim + avatar 28px; hedef-band rahat ~40–44px / kompakt ~50–54px; 2-satır bilgi (asset-adı + %24s) KORUNUR — **bilgi-kaybı = fail; 32px-literal sıkıştırma YAPILMAZ; mobil/desktop kırılmaz**; baseline-kırar → before/after zorunlu + Dock proof-surface materyali + **K-C2 shared-element = S4 karar-kapısı** (bu sprintte yeniden sunulur; öncesinde kapalı).
5. **S5 · Landing Hero Cinematic** — **hero-v3-kanıt-rasathanesi** (yalnız CSS/Canvas-2D; 3D şeridi KAPALI). *(KİLİTLİ — atlanamaz.)*
6. **S6 · Landing Scrollytelling** — anlatı-kavisi (**Hero → Proof → Signal-Lifecycle → Trust → CTA**) + CSS scroll-driven/IO + mobil fallback; yalnız landing, app'te scroll-jacking = 0. *(KİLİTLİ — atlanamaz.)*
7. **S7 · Polish & Perf Sweep** — optik-hiza denetimi · Lighthouse/LCP/CLS · reduce iki-yön · 3-viewport · amatör-detay av listesi.

Her sprint: salt-okuma analiz → onay → küçük commit'ler → **gerçek görsel-QA (desktop + mobil/dar-viewport + scroll + popup/modal + animation-timing + console + premium-kalite + amatör-detay listesi)** → ayrı push onayı. Ayrıntı: [REDESIGN-V3-PLAN](./REDESIGN-V3-PLAN.md).

### pv-dod · §03-K DoD (test-edilebilir)

- App-viewport squint: en parlak ≤2 = {kahraman-rakam, birincil-CTA}.
- Ekran başına **well ≤2**; well-dışı inset-hairline=0 (craft-veri-kuyusu ölçümü AYNEN).
- `backdrop-blur` yalnız E3 overlay; yüzey-kartı blur=0.
- Ekran başına **kahraman-rakam=1**; makbuzsuz kahraman=fail; count-up=fail.
- `<Karot`=0 · yeni glyph/sembol=0 · yeni hex/süre-token=0 (mevcut lint setleri geçerli).
- Animasyonlu skeleton=0; hairline-iskelet CLS=0.
