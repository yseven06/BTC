...
| **App** (dashboard/signals/markets…) | micro: hover `--dur-warm 140` · state geçişi `--dur-state 180` · sinyal-Karot skeleton · foton-tick `--dur-photon 150` · **route geçişi `--dur-route 180` ışık-devri, layout-anim yok** | ambient · paralaks · scroll-reveal · **count-up** · sayfa-içi dekoratif döngü |
...
2. **Karot zorunluluğu.** Her sinyal yüzeyinde Karot; grep Karot-suz sinyal-yüzeyi = 0. Karot her ölçekte 2D SVG (16/48/200); three/R3F initial = 0.
...
4. **Squint testi.** Hero/viewport'ta blur(~8px)+eşikleme sonrası en parlak 2 öge = {Karot, CTA}; dekoratif luminans ≤ veri luminansı. (Ambient glow-drift daima loş/yönlü, vurgu ≤2 bütçesine girmez.)
