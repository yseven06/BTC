...
// Read-only top-N teaser: symbol · direction · live-phase · live price · conviction.
      ...
      <div className="rounded-card border border-border-subtle divide-y divide-border-subtle overflow-hidden">
        {Array.from({ length: limit }).map((_, i) => (
          <div key={i} className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="h-3 w-14 rounded bg-white/[0.05] ml-auto" />
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div className="rounded-card border border-border-subtle divide-y divide-border-subtle overflow-hidden">
      {rows.map((sig) => {
        ...
        const conf = sig.confidence_score ? Math.round(sig.confidence_score) : null;
        return (
          <button
            key={sig.id}
            type="button"
            onClick={() => onSelect(sig)}
            className="focus-ring w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-e-2/50"
          >
            ...
            <span className="ml-auto flex items-center gap-3 tabular-nums">
              ...
              {conf != null && (
                <span className="hidden sm:inline text-micro text-text-muted">güven {conf}</span>
              )}
            </span>
          </button>
        );
