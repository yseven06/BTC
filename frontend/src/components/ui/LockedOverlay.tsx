  ...
  return (
    <div className={`absolute inset-0 flex flex-col items-center justify-center bg-bg-primary/85 backdrop-blur-md rounded-2xl z-20 p-6 text-center ${className ?? ''}`}>
      ...
      {PAYMENTS_ENABLED ? (
        <Link
          href="/pricing"
          className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-amber hover:bg-amber/90 text-e-0 text-sm font-bold transition-colors"
        >
...
export function InlineLocked({ feature = 'Pro' }: { feature?: string }) {
  return (
    <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase text-amber bg-amber/10 border border-amber/30 px-2 py-0.5 rounded">
