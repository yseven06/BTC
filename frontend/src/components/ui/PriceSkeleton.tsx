 ...
 * a static hairline placeholder during a short grace window (--hl10 band, no
 * pulse/shimmer — §03-K pv-yükleme), then falls back to "—" only if the price
...
export function PriceSkeleton({ graceMs = 300 }: { graceMs?: number }) {
  ...
  return (
    <span
      ...
      className="inline-block h-3.5 w-14 rounded bg-[var(--hl10)] align-middle"
    />
  );
}
